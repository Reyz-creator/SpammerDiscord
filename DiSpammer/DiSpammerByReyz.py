...
system('title [DISPAMMER] - Discord Spammer By Reyz')
lock = Lock()
session = Session()
...
class Main:
    ...
    def join_title(self):
        while self.variables['completed'] < len(self.variables['tokens']):
            ...
            sleep(0.4)
        system('title [DISPAMMER] - Discord Spammer By Reyz ^| Token: %s/%s (%s%%) ^| Retries: %s' % (self.variables['completed'], len(self.variables['tokens']), round(((self.variables['completed'] / len(self.variables['tokens'])) * 100), 3), self.variables['retries']))

    def spam_title(self):
        while True:
            system('title [DISPAMMER] - Discord Spammer By Reyz ^| Sent: %s ^| Retries: %s' % (self.variables['sent'], self.variables['retries']))
            sleep(0.4)
